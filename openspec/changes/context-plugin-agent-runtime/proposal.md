...
7. 支持 Agent 基于复制来源应用、当前前台应用、内容类型和用户确认记录生成插件草稿与优先级建议。
...
- `Ctrl/Cmd+J` 对链接默认打开目标，对普通文本默认下钻详情页，对其它内容按插件匹配优先级触发最合适动作。
- Agent 可以学习“从哪个应用复制、当前在哪个应用、用户最后选择了哪个插件”，生成或调整插件触发优先级，但所有固化都要可回滚。
...
- `Ctrl/Cmd+J` 动作解析器能返回候选插件、命中原因、优先级和是否需要确认。
