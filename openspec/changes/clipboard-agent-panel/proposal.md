...
ClipForge 的核心定位仍然是快速剪贴板工具。用户复制链接、文章、代码、错误日志、文件路径或图片后，当前应用已经能沉淀历史与上下文，但还缺少一个轻量入口把剪贴板上下文、收藏、搜索结果、文件和私域 skill 上下文快速交给 Agent 处理，并把处理结果回到剪贴板工作流里。

这里的 Agent 能力不是 Agent 管理平台。ClipForge 不负责管理多个 Agent、不做团队协作、不做远程机器人、不做 Agent 生命周期控制台；ClipForge 提供的是悬浮窗内的 Agent 工作面板，把剪贴板私域上下文快速组织成可引用输入，调用本地 CLI Agent、AI SDK/OpenAI-compatible provider 或 MCP 工具，让 Agent 为剪贴板内容处理和剪贴板工具闭环服务。
...
1. 在快速面板或详情页增加一个完整但克制的 Agent 工作面板，用于围绕剪贴板私域上下文完成对话、处理、保存、剪贴板管理和工具调用闭环。
2. 新建对话时默认附带当前剪贴板条目，但用户可以选择 `当前 / 指定条目 / 收藏 / 搜索结果 / all / 文件 / skill 上下文` 作为引用集合，而不是把完整正文盲目塞进 prompt。
3. 支持调用一个最小本地 CLI Agent adapter，例如已配置的 `claude -p`、`codex` 或后续可检测命令模板。
...
11. Agent 面板可以作为 ClipForge 自定义能力入口：集成标准 skill、私域 skill、MCP tool 调用、剪贴板管理动作、分析回填动作和能力草稿保存。
...
- 用户也可以把引用范围切换为全部最近剪贴板、某几条历史、收藏条目、搜索结果、文件或某个剪贴板私域 skill 输出的上下文。
...
- 用户可以在 Agent 面板中调用标准 skill 或 MCP tool 来管理 ClipForge，例如批量整理标签、分析一组收藏、生成摘要并回填、创建剪贴板处理 skill 草稿。
- 面板关闭或隐藏不打断长任务，用户稍后打开仍能看到运行状态和结果。
- 外部 Agent 通过 MCP 调用 ClipForge 时，只能使用受控剪贴板工具，边界清晰、可审计。

## 成功标准

- 快速面板存在 `剪贴板 / Agent` 两个清晰入口，Agent 页不影响默认剪贴板列表操作。
- 从任意当前条目进入 Agent 页时，都会生成默认 `AgentContextSet`，其中第一项是当前 clip 引用；用户可以追加、移除或替换为 all、收藏、搜索结果、指定条目、文件或 skill 上下文。
...
- Agent 面板能展示可用 ClipForge 能力入口，包括剪贴板查询、标签维护、收藏/归档、结果回填、skill 草稿、MCP tool 调用；危险动作必须预览并确认。
