...
`settings-field-refactor` 的评审结论已合并到本提案：采用最小 `SettingFieldConfig` catalog，但不采用 `json-render`、`@json-render/*`、`zod` 或通用 schema-driven renderer。catalog 只描述 UI 放置、控件类型、i18n key 和排序，不定义默认值、schema 校验、写入策略或 provider 解析。
...
截至 2026-07-15 的 live checkout：

- 已 vendored：Tabs（`src/components/animate-ui/components/animate/tabs.tsx`）、Tooltip（`src/components/animate-ui/primitives/animate/tooltip.tsx`）、Toggle Group（`src/components/animate-ui/components/animate/toggle-group.tsx`）。
- 待 vendor：Animate UI Radix Sidebar、Animate UI Code Tabs。
- 已有依赖：`radix-ui`、`motion`、`@floating-ui/react`、`lucide-react`，当前不需要为了最小 catalog 决策新增包。
- Context7 重新拉取 Animate UI 文档仍被 monthly quota 阻塞；实现 Sidebar / Code Tabs 前需在配置 `CONTEXT7_API_KEY` 或登录后重试。
