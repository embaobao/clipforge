...
本提案只做“Agent 调用为剪贴板和 ClipForge 能力服务”，不做“Agent 管理”。产品结构上，Agent 是悬浮面板里的极简聊天框；技术结构上，Agent 是 App Core 的一个受控调用方；权限结构上，Agent 只能通过上下文引用集合、上下文快照和明确工具接口读写数据。

UI 设计遵循工具型桌面应用原则：紧凑、低干扰、可键盘操作、状态明确。参考 `ui-ux-pro-max` 的交互建议，面板需要有清晰焦点态、加载态、取消态、错误态、可恢复状态，避免复杂 onboarding 和装饰性动效。首屏只允许出现消息区、引用附件条、Agent/CLI 选择、输入框和发送/停止按钮；旧的模式 tab、skill 输入、能力工具栏必须移出首屏。
...
- 顶部只保留极简标题、Agent/CLI 选择和返回剪贴板按钮；不再展示大标题、状态说明和复杂工具按钮。
- 当前 clip 作为默认附件 chip 显示在 composer 上方；其它引用通过 `@` 自动补全、添加引用按钮或命令菜单加入。
- 中间展示对话消息、工具调用、运行状态和错误；空状态只显示一行轻提示，不能做工作台说明。
- 底部是输入框，默认带上下文引用集合，不把完整正文直接插入输入框。
- 结果消息上提供动作：复制、保存为新条目、收藏来源、归档来源、追加 tag、重新运行、停止。
- 标准 skill、私域 skill、MCP tool、标签管理、收藏/归档、分析回填默认隐藏到命令菜单或结果动作，不占据首屏。
...
<AgentChatPanel>
  <header>
    <select aria-label="选择 Agent">{providers}</select>
    <button aria-label="返回剪贴板" />
  </header>
  <MessageViewport />
  <ReferenceAttachmentBar />
  <Composer>
    <button aria-label="添加引用" />
    <textarea placeholder="@ 引用剪贴板，输入问题..." />
...
- 不出现 `当前 / 指定 / 收藏 / 搜索 / All` 模式 tab；这些范围只作为添加引用弹层里的 scope 候选，不在首屏常驻。
- 不出现独立 Skill 名称输入框；保存 skill 只能通过命令或结果动作进入。
- 不出现“摘要 / 标签 / 解析 / 存 Skill”首屏工具栏；常用意图可以作为空状态建议或命令补全。
- 引用 chip 单行横向滚动，最多显示标题、类型、权限和移除按钮。
- `@` 自动补全列表最多展示 6 项，来源包含当前条目、已选条目、当前搜索结果、收藏、最近历史；回车或点击即添加引用，不直接发送。
- Agent/CLI 选择是紧凑 select 或 combobox，展示 provider label 和 readiness；检测中不阻塞输入。
- 所有控件保持 4/8px spacing 和 4-6px radius，不能出现卡片套卡片。
...
- React DOM 必须显式保留 `provider / scroller / viewport / content / item / button` 这六层语义，方便后续替换为真正的 shadcn Base `MessageScroller` 或 AI SDK UI renderer。
- `MessageScrollerItem` 负责稳定 `messageId`、`rowId`、`scrollAnchor` 和数据属性；消息正文渲染在 item 内部的 body 层，避免工具展开、结果动作和状态变化改写滚动锚点。
- run marker 也必须走同一套 `MessageScrollerItem -> body` 结构，不能直接裸渲染到 content 内，否则运行状态会绕过滚动锚点和样式约束。
- `MessageScrollerButton` 使用稳定 `data-message-scroller-button` 标记，当前实现对应“跳到最新”，后续替换 shadcn Base 原语时不改消息区层级。
- 用户消息可以是紧凑右侧气泡；Agent 消息保持左侧正文流，不做大面积卡片；run marker 是居中的短状态行。
- `MessageScrollerButton` 对应“跳到最新”，只在离开 live edge 或有新内容时出现，不常驻占位。
...
- 用户离开 live edge 后记录当前可见 `data-agent-row-id` 和行内 offset；停止、错误、确认区或历史消息合并造成 DOM 变化时，优先按该 row 恢复阅读位置。
...
- 引用附件固定在 composer 上方，单行横向滚动，长预览折叠；它是本次发送的上下文，不是聊天消息气泡。
...
- 结果动作跟随对应 assistant 消息渲染，不再在 composer 上方额外铺一条全局结果工具栏，避免聊天框首屏被动作区挤乱。

### Attachment 参考方案

引用附件参考 shadcn `Attachment` 的 media/title/description/actions 结构，但保持 ClipForge 的紧凑尺寸：

- 每个引用 chip 固定宽度，包含类型图标、标题、`payloadKind - permissionScope` 描述和移除按钮。
- 代码实现应复用本地 Base `Attachment` 原语：`AttachmentMedia` 承载类型图标，`AttachmentContent` 承载标题和权限描述，`AttachmentActions` 承载移除动作；Agent 面板只覆写尺寸、间距和颜色 token。
- 图标按 `image/file/link/code/json/command/text` 等类型区分，不能只用单一文档图标表达所有内容。
- 移除按钮必须有 `aria-label`，按钮点击只移除下一次 run 的上下文，不删除剪贴板条目。
- 附件条保持单行横向滚动，不能撑高 composer，也不能和消息区形成卡片套卡片。
- 文件、图片和 skill 上下文默认只展示元数据；读取正文、OCR 或文件列表仍需单次授权。

### 页面拆分与 Motion 动效

Agent UI 必须拆成可维护的页面边界：

- `ClipboardAgentPanel` 是容器层，只负责 Tauri command、事件监听、run 状态、上下文集合和结果回调。
- `AgentChatPage` 是独立页面层，只消费渲染模型和事件回调，负责消息区、附件条、composer、引用候选和结果动作。
- 消息行、附件 chip、引用 scope、工具调用预览和自定义事件预览是 `AgentChatPage` 内部原语，不能再回流到容器层。
- 容器层计算 `active/disabled/count` 等状态，页面层不直接读取 `removedReferenceIds`、`scopeRequests` 或 provider detect 细节。
- 后续如果替换为 shadcn Base `MessageScroller`、更完整的 Attachment 原语或 AI SDK UI renderer，只替换页面层和原语，不改 runtime 容器。

动效只使用 `motion` 包和 `motion/react`：

- 消息新增、附件增删、引用候选弹层和“跳到最新”按钮使用 `AnimatePresence`。
- 引用范围的当前选中边框使用 `layoutId` 共享布局，不手写坐标或动画 `top/left/width/height`。
- 运行状态图标可以做轻量旋转，但必须在 reduced motion 下静止。
- 所有进入/退出动效只使用 opacity、transform 或 Motion layout；不动画高度、margin、padding。
- Motion 只放在 `AgentChatPage` 这个叶子页面中，避免主剪贴板列表引入额外重渲染。

`maisano/react-router-transition` 的模式可以借鉴，但不作为依赖引入：

- 该库面向 `react-router-dom@5.x` 的路由切换，当前项目使用 TanStack Router 和面板级 tab/surface 切换，不匹配。
- 该库依赖 `react-motion` 的旧式 spring API，不能作为 React 19 + Motion 的主路径。
- 可借鉴的是“旧页面和新页面同时存在、绝对定位、出入场分离”的理念；在本项目中用 `AnimatePresence` 和稳定 page shell 实现。
...
- 默认引用 `current`；`当前 / 已选 / 收藏 / 搜索结果 / all / 文件 / skill 上下文` 只在添加引用弹层中作为 scope grid 出现，点击后生成对应引用 chip。
...
标准协议兼容目标按 AI SDK v5 / OpenAI-compatible 的常见抽象建模：
...
- Rust 侧维护 provider registry，至少包含 `local-cli` 和 `openai-compatible` 两类 provider。
- OpenAI-compatible 配置从原生运行环境读取：`CLIPFORGE_AGENT_OPENAI_BASE_URL`、`CLIPFORGE_AGENT_OPENAI_MODEL`、`CLIPFORGE_AGENT_OPENAI_API_KEY`。
- React 只能拿到 `baseURL/modelId/apiKeyRef` 的脱敏摘要，不能拿到 API key 原文。
- OpenAI-compatible transport 输出标准 JSON event lines，Rust runtime 归一化为 `TEXT_MESSAGE_CONTENT`、`TOOL_CALL`、`TOOL_RESULT` 和 `data-status` parts。
- 本地 CLI stdout/stderr 和 OpenAI-compatible stream 最终都通过同一个 `agent_ui_message` 事件进入同一消息渲染器。
