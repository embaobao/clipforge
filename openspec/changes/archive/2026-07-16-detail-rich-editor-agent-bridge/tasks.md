...
### Phase 8 复跑记录（2026-07-16）

- 已复跑 `node scripts/verify-editor-agent-bridge.mjs`：通过，覆盖 editor suggestion、preview patch、变量上下文和 Agent 建议降级相关静态/契约断言。
- 已运行 `pnpm test:unit`：通过，其中包含 editor-suggestion、editor-action、sensitive checks 和 bridge verifier。
- 已运行 `pnpm openspec validate detail-rich-editor-agent-bridge --strict`：通过。
- 已补齐详情编辑器 `保存并复制` 入口：保存成功后使用 `save_editor_draft` 返回的当前 clip 走 `onCopy(postSaveClip)` 写回系统剪贴板路径，避免 `copyText` 重新 capture 成新历史条目；并为按钮补齐中英文文案。`verify-editor-agent-bridge.mjs` 已新增断言，确保 copy 发生在 `save_editor_draft` 完成之后且不走 `onCopyText`。
- 已复跑 `node scripts/verify-editor-agent-bridge.mjs`、`pnpm check:i18n`、`pnpm exec tsc --noEmit`、`pnpm test:unit`、`pnpm openspec validate detail-rich-editor-agent-bridge --strict`：通过。
- 剩余 4 项仍需真实 `pnpm tauri dev` 下的详情页编辑保存、Markdown 取消、保存并复制、保存并粘贴链路证据；本轮不勾选。
