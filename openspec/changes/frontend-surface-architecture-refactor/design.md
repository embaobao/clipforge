...
- `PanelHeader` 固定在顶部，保留可拖拽留白；搜索在用户输入或按下 `/` 后才展开并接管焦点，右上角 Logo/Menu 触发器只承载低频动作。
...
- 面板打开时不默认展示或聚焦搜索；用户输入或按下 `/` 后，搜索输入保持焦点稳定，结果直接刷新列表。
- 虚拟列表中的图片预览、上下文菜单等跨行浮层必须使用页面顶层 portal，不能依赖放开行级 `overflow` 规避裁切。
