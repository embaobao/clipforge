...
P1。该提案是“格式支持”的基础层，应在 GitHub 分发与检查升级能力之后推进。它负责把 ClipForge 从纯文本剪贴板升级到图片、文件、HTML/RTF 等多格式采集与基础写回，为后续保真复制、搜索筛选和智能解析提供结构化字段。
...
4. **类型采集配置**：允许用户启用/禁用文本、HTML、RTF、图片、文件采集；第一阶段使用固定 `primaryFormat` 选择规则，同时保留同一次复制中的所有可用 representation。
