...
- [x] 图片行只显示图片文件名，完整路径移入详情元数据
- [x] 图片缩略图固定为 24px，并与文本行垂直对齐
- [x] 图片缩略图使用页面顶层 portal 展示最大 128×128px 的等比例 hover 预览，避免虚拟列表裁切
- [ ] 在真实 Tauri 窗口由用户确认图片 hover 预览完整显示、不会遮挡或截断关键列表内容
