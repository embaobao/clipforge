...
- 图片条目：
  - 快速面板保持高密度行高，不把缩略图做成素材库卡片；左侧使用固定 24px 缩略图或图片图标，主文案只显示图片文件名并允许中间省略，不显示完整路径、尺寸或文件大小。
  - 缩略图 hover 显示最大 128×128px、等比例的图片预览。预览必须通过页面顶层 portal 定位，不能作为虚拟列表行的子节点，以免被滚动容器或固定行高裁切。
  - 详情页展示原图预览、图片文件名、完整路径、尺寸和文件大小。
- 文件条目：
  - 单文件：显示文件图标 + 文件名 + 路径。
